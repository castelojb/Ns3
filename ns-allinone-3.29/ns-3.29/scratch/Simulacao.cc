...
main (int argc, char *argv[])
{
  ...
  std::string transportProt = argv[1];
  std::string socketType;
  string velocidade = argv[2];
  string iteracao = argv[3];
  string pasta = transportProt;
  CommandLine cmd;
  ...
  pointToPoint.SetDeviceAttribute ("DataRate", StringValue (velocidade));
  ...
  pointToPoint.EnableAsciiAll (ascii.CreateFileStream (pasta+"/"+transportProt+"-Conexão-"+velocidade+"-"+iteracao+".tr"));  
